...
2. Допустимые "label" и замкнутые значения "type":
   - Перечень допустимых значений type для каждого label является СТРОГО ЗАКРЫТЫМ (Closed Enum).
   - Модели КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО придумывать свои типы (такие как "supplement", "food", "cosmetics", "drink", "medicine", "herb"). Если объект не укладывается ни в один из допустимых типов -> его КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО извлекать в entities.
...
3. Zero Noise Guard — Product & Entity Filter (ЖЕСТКИЕ ПРАВИЛА ФИЛЬТРАЦИИ):
   - Product извлекается ИСКЛЮЧИТЕЛЬНО при наличии зарегистрированного бренда или коммерческой торговой марки производителя (например: "Solgar Astaxanthin", "Now Foods Omega-3", "Dyson Airwrap", "iPhone 16 Pro", "Lego Technic 42115", "Курс 'Профессия Python-разработчик'").
   - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО извлекать как Product, так и как Entity:
     * Биологически активные добавки (БАДы), витамины, минералы, травы, растительные экстракты, эфирные и базовые масла, суперфуды, лекарственные молекулы, пептиды и кислоты БЕЗ указания конкретного коммерческого бренда и торговой марки производителя (например: астаксантин, чёрный тмин, масло зверобоя, хлорофилл, магний, витамин D3, Омега-3, цинк, коллаген, спирулина, куркумин, креатин, ашваганда). Контекст их приема передавай ИСКЛЮЧИТЕЛЬНО через корневой список microconcepts (например: 'Dietary Supplements', 'Herbal Medicine', 'Nutritional Biohacking').
...
5. Поле "micro_concepts":
   ...
   - Примеры антипаттернов (Bad):
     ...
     * name: "PostgreSQL" -> micro_concepts: ["Postgres", "SQL"] (ОШИБКА: тавтология и дублирование имени)
   ...
   - Примеры правильного извлечения (Good):
     ...
     * name: "PostgreSQL" -> micro_concepts: ["Relational Databases", "Data Storage Architecture"]
...
8. Ограничения:
    - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО извлекать автора публикации, указанного в блоке контекста <author> (его title, handle, вариации имени и транслитерации), как сущность Entity, Organization или Product. Автор задан в контексте как $AUTHOR и адресуется исключительно через relations.
...
1. "language": двухбуквенный строчный ISO 639-1 код ("ru", "kk", "en", "uk", "uz", "de", "es" и т.д.). Иерархия определения:
   - Если <caption-text> содержит 3 и более осмысленных слова -> язык определяется строго по <caption-text>.
   - Если <caption-text> отсутствует или содержит менее 3 слов, но присутствует <transcription-text> (3 и более осмысленных слова) -> язык определяется по <transcription-text>.
   - Если суммарно в тексте менее 3 осмысленных слов (музыка, шумы, эмодзи, мусорные теги) -> строго null.
   - Запрещено возвращать "kz" вместо "kk" для казахского языка (строго "kk").
   - Присутствие терминов, названий библиотек, брендов на латинице или хештегов внутри русскоязычного текста не делает пост английским. Язык определяется по связующей речи и грамматической структуре.
...
4. Нейромедиаторные триггеры (шкала от 0.0 до 1.0; 0.0 -> триггер отсутствует, 0.5 -> умеренный фон, 1.0 -> доминирующий посыл):
  ...
  Правила определения:
    - Если у поста есть осмысленный текст -> ты ОБЯЗАН оценить каждый скор (от 0.0 до 1.0), при этом как минимум 1–2 ведущих триггера ОБЯЗАНЫ иметь ненулевой скор >= 0.3.
    - Если у поста нет осмысленного текста (менее 3 слов) либо пост является спамом/скамом (is_spam_or_gambling=true) -> все 6 скоров строго 0.0.
</psychographics-rules>
...
{{
  ...
  "relations": [],
  "microconcepts": [],
  "psychographics": {{"language": "ru", "tone": null, "secondary_tone": null, "score_dopamine": 0.0, "score_oxytocin": 0.0, "score_serotonin": 0.0, "score_cortisol": 0.0, "score_adrenaline": 0.0, "score_endorphin": 0.0}},
