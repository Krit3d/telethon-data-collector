...
class QueryParser:
    ...
    async def parse(self, request: SearchRequest) -> ReformulatedQuery | None:
        ...
        system_prompt = (
            ...
            "- ONTOLOGICAL BOUNDARY RULE (CRITICAL): Never extract standalone words that denote abstract workflow stages, execution phases, deployment environments (e.g., production/prod, staging), software lifecycles, media formats, or container types. In technical software contexts, words indicating implementation, deployment, production, or testing describe WHERE or HOW software is deployed, NOT WHAT the core technology subject is. Such environment/lifecycle terms MUST ONLY exist as part of compound entities (e.g., 'финтех-продакшен'). Standalone entities are strictly reserved for unambiguous domain subjects, technologies, methods, and niche topics."
            "3. target_topics: Extract 2-4 standard IAB 3.1 category string names in English matching query intent. For multi-domain queries (e.g., technology in finance), include IAB categories representing both the technology domain and the target industry domain.\n"
